  ...
  model {
    ...
    talentStreams = softwareSystem "TalentStreams" "Next.js-приложение: публичный сайт с подборками кандидатов, формами регистрации и редактором выпусков" {
      ...
      webApp = container "Web Application" "Server-side рендеринг, Server Actions, интеграции с Google Sheets и SendPulse" "Next.js 14, TypeScript" "WebApp" {
        ...
        # Server Actions
        serverActions = component "Server Actions" "registerEmployer() — пишет в Sheets со статусом «На проверке».\nregisterCandidate() — пишет в Sheets с id = crypto.randomUUID() (сгенерирован здесь же, синхронно); если передан resumeUrl+resumeKind — пишет первую версию резюме в Neon (candidateResumes, TASK-32).\npublishMailingList() — создаёт кампанию в SendPulse, тело письма без точного счётчика кандидатов (TASK-26).\nconfirmEmployer() — syncEmployerToSendPulse(), затем статус «Подтверждён» в Sheets.\nrejectEmployer() — если запись была «Подтверждён», сначала removeEmployerFromSendPulse() (полная отписка от мастер-книги и всех книг текущих стримов), затем статус «Отклонён» в Sheets; для «На проверке» — только смена статуса.\ndeleteEmployer(rowIndex) — та же условная отписка от SendPulse, что у rejectEmployer(), затем deleteEmployerRow() физически удаляет строку из Sheets.\napproveCandidate() / rejectCandidate() — статусы кандидата в Sheets.\naddAllColumns() — миграция колонок (Sheets).\nsubmitContactRequest() — записывает запрос по кандидату в Neon.\nsubmitGeneralInquiry() — записывает общий запрос (candidateId='') в Neon.\nsetContactRequestStatus(id, status) — меняет статус запроса в Neon по id (UUID).\nupdateStream(id, data) / createStream(data) / deleteStream(id) — CRUD стримов в Neon (revalidatePath /editor/streams).\nupdateCandidate(rowIndex, data) — обновляет поля кандидата (включая title/summary/stream/level/countryPrimary/countryDesired/activeSince) в Sheets через updateCandidateFields(); если resumeUrl реально изменился (data.resumeChanged передан клиентом) — доп. пишет новую версию резюме в Neon (revalidatePath /editor/candidates).\ngetCandidateResumeHistory(candidateId) — история версий резюме кандидата из Neon, новые сверху (TASK-32).\nupdateEmployer(rowIndex, data) — обновляет поля работодателя в Sheets через updateEmployerFields(); если работодатель уже подтверждён, доп. синхронизирует правки в SendPulse через syncEmployerToSendPulse() (revalidatePath /editor/employers)." "Next.js Server Actions" "Logic"
