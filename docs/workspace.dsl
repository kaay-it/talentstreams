workspace "TalentStreams" "Платформа подборки проверенных кандидатов для работодателей Центральной Азии" {
  ...
  model {
    ...
    neon = softwareSystem "Neon (PostgreSQL)" "Serverless PostgreSQL через Vercel Marketplace. Таблицы: contactRequests (FK на employers/streams), streams (+status: Активный/Архивный), candidateResumes (TASK-32 — история версий резюме кандидата), employers (TASK-DB-3), mailingListEntries (TASK-DB-5, streamId — FK на streams.id, ON DELETE SET NULL). Подключение через neon-http driver (HTTP, без WebSocket)." "External"
    ...
    talentStreams = softwareSystem "TalentStreams" "Next.js-приложение: публичный сайт с подборками кандидатов, формами регистрации и редактором выпусков" {
      ...
      webApp = container "Web Application" "Server-side рендеринг, Server Actions, интеграции с Google Sheets и SendPulse" "Next.js 14, TypeScript" "WebApp" {
        ...
        releaseCreateModal = component "ReleaseCreateModal" "Самодостаточная модалка создания выпуска (свой триггер-кнопка + open-состояние — родитель ReleasesPage серверный, без обёртки-клиента): выбор стрима и даты → число подходящих кандидатов/подтверждённых подписчиков + предупреждения (< 6 / > 10, 0 подписчиков) → предзаполненный чекбокс-список кандидатов (getEligibleCandidatesForRelease, все выбраны по умолчанию), рядом — схлопнутый список «Кандидаты на паузе, дата не наступила» (getPausedCandidatesForRelease, TASK-05, закрыт по умолчанию, чекбоксы на той же переменной selectedIds — editor может включить вручную при нехватке подходящих) → createMailingList(). В обоих списках рядом с кандидатом показана activeSince. Кандидаты урезаны до id/name/title/level/activeSince — остальные поля Profile в клиентский бандл не попадают." "React Client Component" "UI"
        ...
        streamsPage = component "StreamsPage (/editor/streams)" "Редактор стримов: список Stream / Тип / Описание из Neon. Полный CRUD через inline-редактирование (StreamsTable) для активных стримов, удаление и добавление новых строк. Колонка «Кандидаты» (TASK-27) и «Подписчики» — число подтверждённых работодателей на стрим (confirmedEmployersForStream, lib/db/employers.ts), оба счётчика считаются в памяти на одном запросе, без отдельного похода в БД на стрим. Доп. getMailingLists()+getCampaigns() на всю страницу — группирует выпуски по streamId в mailingHistoryByStreamId (с пометкой sent), делит стримы на activeStreams/archivedStreams по status. Рендерит StreamsTable (активные) и ArchivedStreamsSection (архивные, ниже). Пункт «Стримы» в EditorNav." "Next.js Server Component" "Page"

        archivedStreamsSection = component "ArchivedStreamsSection" "Отдельная схлопнутая секция страницы стримов («Архивные стримы (N)», закрыта по умолчанию, null если архивных нет) — не часть строки, как ReleaseDeleteButton, а весь блок целиком. Упрощённая read-only таблица (без inline-редактирования — архивный стрим не редактируется, только восстанавливается): ID/Название/Тип/Кандидаты/Подписчики/Описание + кнопка «Восстановить» (unarchiveStream(id), без подтверждения). Та же раскрываемая история рассылок на строку, что в StreamsTable." "React Client Component" "UI"
        ...
        # Integrations
        sheetsLib = component "Sheets Library (lib/sheets.ts)" "Весь доступ к Google Sheets через Service Account JWT.\nАутентификация: safeJsonParse() устойчив к сырым переносам строк в private_key GOOGLE_SERVICE_ACCOUNT_JSON (артефакт vercel env pull); ошибки конфигурации — конкретные (что не задано/невалидно), не общая фраза.\nЧтение: профили кандидатов. Стримы (TASK-DB-8), работодатели (TASK-DB-3) и подборки (TASK-DB-5) перенесены в Neon.\nЗапись: регистрации кандидатов (appendCandidateRow вызывает ensureCandidateColumns автоматически, возвращает { rowIndex } из updates.updatedRange — нужен createCandidate() для донаполнения строки сразу после вставки, TASK-33); статусы и поля кандидата (updateCandidateStatus, updateCandidateFields, включая title/summary/stream/level/countryPrimary/countryDesired/activeSince).\nУдаление: deleteCandidateRow(rowIndex) (TASK-33) — batchUpdate/deleteDimension, целится в первый лист таблицы (sheets[0]), т. к. диапазоны кандидатов нигде не квалифицированы именем листа.\nАвто-миграция колонок кандидатов: ensureCandidateColumns() — вызывается внутри appendCandidateRow() при каждой записи, не разовая ручная операция.\nАвтосоздание листов через ensureSheet().\ncandidateMatchesStream() / getCandidatesForStream() — сопоставление кандидата стриму по колонке Stream (multi-select), без учёта регистра (TASK-27 ✅).\nparseRuDate() / getEligibleCandidatesForRelease() — кандидаты стрима со статусом «Активный» (уже гарантирован getProfiles()) и уже наступившей/пустой датой Active Since (TASK-28, закрыта TASK-05 — activeSince теперь обновляется автоматически); parseRuDate() также используется lib/db/mailing-lists.ts для одноразовой конвертации ru-RU→ISO в бэкафилле (TASK-DB-5).\ngetPausedCandidatesForRelease() (TASK-05) — зеркало getEligibleCandidatesForRelease() с обратным условием (activeSince ещё не наступил) — схлопнутый список «на паузе» в ReleaseCreateModal, editor может включить кандидата вручную.\nCandidateStatus получил значение \"Не публиковать\" (TASK-05) — терминальный статус, выставляется автоматически из publishMailingList() (app/actions.ts) после 3-й реальной отправки кандидата; getCandidates() (все статусы, с rowIndex — нужен для updateCandidateStatus()) и updateCandidateStatus(rowIndex, status, activeSince) — уже существовавшие функции, переиспользованы без изменений сигнатуры.\nContact Requests, Streams, Employers и Mailing Lists перенесены в Neon → lib/db/contact-requests.ts, lib/db/streams.ts, lib/db/employers.ts, lib/db/mailing-lists.ts (TASK-DB-5)." "TypeScript, Google Sheets API v4" "Integration"

        dbLib = component "DB Library (lib/db/)" "Drizzle ORM + @neondatabase/serverless (neon-http driver).\nschema.ts — схемы таблиц contactRequests, streams (id/name/type/description/status), candidateResumes (TASK-32), employers (TASK-DB-3) и mailingListEntries (TASK-DB-5) — id/listId/streamId (FK, ON DELETE SET NULL)/targetDate/candidateId.\nindex.ts — клиент drizzle(neon(DATABASE_URL)).\ncontact-requests.ts — appendContactRequest(), getContactRequests(), updateContactRequestStatus(id).\nstreams.ts — getStreams() (только status='Активный' — архивный стрим не предлагается ни в одном месте, где выбирается стрим для нового действия), getStreamsDetailed() (все, только для страницы стримов), getStreamById(id), createStreamRecord(), updateStreamRecord(id, Partial<{name,type,description,status}>) (объединяет правки полей и смену статуса в одну функцию — тот же приём, что updateEmployerFields), deleteStreamRecord().\nmailing-lists.ts (TASK-DB-5) — getMailingList(listId) (джойн на streams по streamId + джойн с getProfiles() из lib/sheets.ts — кандидаты не переехали), getMailingListMeta(listId) (лёгкий вариант без джойна на кандидатов, для publishMailingList()/deleteMailingList()/submitContactRequest()/submitGeneralInquiry() — им нужны только stream/date), getMailingLists() (сортировка по убыванию targetDate — прямое сравнение ISO-строк, без parseRuDate; MailingListSummary несёт streamId для группировки на странице стримов), createMailingListRows({stream, date, candidateIds}) (stream — имя, резолвится в streamId через getStreamIdByName() перед вставкой; date — ISO с <input type=\"date\">, пишется напрямую в нативную колонку targetDate), deleteMailingListRows(listId), getMailingListCandidateIds(listId) (TASK-05, пауза публикации — только candidateId одного выпуска, без джойна на streams/Profile, для applyPublicationPauseRules() в publishMailingList()), getMailingListsForCandidate(candidateId) (TASK-05 — все выпуски конкретного кандидата, новые сверху; прямой WHERE candidateId = ..., без похода в Sheets; используется и для видимости в CandidateEditModal, и для подсчёта реальных прошлых отправок при вычислении паузы), updateMailingListDate(listId, date) — снимает плановую дату выпуска на фактическую при первой отправке (publishMailingList(), дата — план, не обязательство), прямой UPDATE targetDate WHERE listId = .... Внешний контракт (все сигнатуры) не поменялся при переходе stream:text→streamId:FK — join происходит внутри модуля, публичный date остаётся ru-RU текстом (isoToRu(), экспортируется — переиспользуется в app/actions.ts вместо ещё одной копии конвертации).\nresumes.ts (TASK-32) — addResumeVersion({candidateId, kind, filename?, url}) (no-op если candidateId/url пустые), getResumeVersions(candidateId) — история версий резюме, новые сверху, getResumeVersionsForCandidates(ids) — то же батчем, deleteResumeVersions(candidateId) (TASK-33) — удаляет всю историю кандидата.\nemployers.ts (TASK-DB-3) — getEmployers(), getEmployerByToken(token) (прямой WHERE token = ..., не выгрузка всех строк с поиском в JS, как было в Sheets), getEmployersByStream(stream), createEmployer(data), updateEmployerFields(token, data) (объединяет прежние updateEmployerStatus+updateEmployerFields в одну функцию — в Sheets они были раздельными только из-за ячеечной записи), deleteEmployer(token), filterCandidatesForEmployer() (чистая функция без I/O, перенесена вместе с типом Employer). streams/additionalCountries — нативные Postgres text[], не comma-joined строка.\nMigrations: scripts/migrate.mjs." "TypeScript, Drizzle ORM, Neon" "Integration"
    ...
    talentStreams.webApp.serverActions -> talentStreams.webApp.sheetsLib "appendCandidateRow(), getCandidates() / updateCandidateStatus() — TASK-05, пауза публикации"
    talentStreams.webApp.serverActions -> talentStreams.webApp.dbLib "createEmployer(), updateEmployerFields(), getMailingListMeta(), createMailingListRows(), deleteMailingListRows(), getMailingListCandidateIds(), getMailingListsForCandidate()"
    ...
    talentStreams.webApp.streamsPage -> talentStreams.webApp.serverActions "updateStream() / createStream() / deleteStream() / archiveStream()"
    talentStreams.webApp.streamsPage -> talentStreams.webApp.archivedStreamsSection "Рендерит"
    talentStreams.webApp.archivedStreamsSection -> talentStreams.webApp.serverActions "unarchiveStream(id)"
    ...
    talentStreams.webApp.streamsPage -> talentStreams.webApp.dbLib "getStreamsDetailed() / getEmployers() — подсчёт подтверждённых подписчиков на стрим / getMailingLists() + история рассылок по streamId"
    ...
    talentStreams.webApp.streamsPage -> talentStreams.webApp.sendPulseLib "getCampaigns() — какие выпуски стрима реально отправлены"
  ...
  views {
    ...
    dynamic talentStreams.webApp "DeleteRelease" "Сценарий удаления ещё не отправленного выпуска редактором" {
      ...
      talentStreams.webApp.dbLib -> neon "SELECT streams.name, targetDate FROM mailingListEntries LEFT JOIN streams ON streamId = streams.id WHERE listId = ... LIMIT 1"
    ...
    dynamic talentStreams.webApp "StreamRenameGuard" "Сценарий: редактор пытается переименовать/удалить стрим, у которого уже есть отправленная рассылка" {
      talentStreams.webApp.streamsPage -> talentStreams.webApp.serverActions "updateStream(id, { name: новое, type, description })"
      talentStreams.webApp.serverActions -> talentStreams.webApp.dbLib "getStreamById(id) — текущее имя для сравнения"
      talentStreams.webApp.dbLib -> neon "SELECT * FROM streams WHERE id = ..."
      talentStreams.webApp.serverActions -> talentStreams.webApp.dbLib "getMailingLists() — все выпуски этого стрима"
      talentStreams.webApp.dbLib -> neon "SELECT ... FROM mailingListEntries LEFT JOIN streams ON streamId = streams.id"
      talentStreams.webApp.serverActions -> talentStreams.webApp.sendPulseLib "getCampaigns() — какие из них реально отправлены"
      talentStreams.webApp.sendPulseLib -> sendPulse "GET /campaigns"
      autolayout lr
    }
    ...
    dynamic talentStreams.webApp "PublishMailingList" "Сценарий публикации выпуска редактором" {
      ...
      talentStreams.webApp.dbLib -> neon "SELECT streams.name, targetDate FROM mailingListEntries LEFT JOIN streams ON streamId = streams.id WHERE listId = ... LIMIT 1"
      talentStreams.webApp.serverActions -> talentStreams.webApp.sendPulseLib "getCampaigns() — первая ли это отправка"
      talentStreams.webApp.sendPulseLib -> sendPulse "GET /campaigns"
      talentStreams.webApp.serverActions -> talentStreams.webApp.dbLib "updateMailingListDate(listId, today) — только если первая отправка и план разошёлся с фактом"
      talentStreams.webApp.dbLib -> neon "UPDATE mailingListEntries SET targetDate = ... WHERE listId = ..."
      ...
      talentStreams.webApp.serverActions -> talentStreams.webApp.dbLib "getMailingListCandidateIds(listId) — только если первая отправка"
      talentStreams.webApp.dbLib -> neon "SELECT candidateId FROM mailingListEntries WHERE listId = ..."
      talentStreams.webApp.serverActions -> talentStreams.webApp.sheetsLib "applyPublicationPauseRules(): getCandidates() — резолвит rowIndex по каждому кандидату выпуска"
      talentStreams.webApp.serverActions -> talentStreams.webApp.dbLib "getMailingListsForCandidate(id) — сколько раз кандидат уже реально отправлялся"
      talentStreams.webApp.dbLib -> neon "SELECT * FROM mailingListEntries WHERE candidateId = ..."
      talentStreams.webApp.serverActions -> talentStreams.webApp.sheetsLib "updateCandidateStatus(rowIndex, status, activeSince) — на каждого кандидата: 1-я отправка +2 недели, 2-я +3 месяца, 3-я «Не публиковать» +10 лет"
      autolayout lr
    }
