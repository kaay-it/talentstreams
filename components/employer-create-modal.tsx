...
export function EmployerCreateModal({
  streams,
  onClose,
}: {
  streams: string[]
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setStatus("error")
      setErrorMsg("Не удалось добавить работодателя")
    }
  }
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto">
      ...
      <div className="flex min-h-full items-center justify-center p-4">
        <div
          role="dialog"
          aria-modal="true"
          aria-labelledby="create-employer-title"
          className="relative w-full max-w-2xl rounded-2xl border bg-card shadow-xl"
        >
          <div className="flex items-center justify-between border-b px-6 py-4">
            <h2 id="create-employer-title" className="text-base font-semibold text-card-foreground">
          ...
          {status === "success" ? (
            <div className="flex flex-col items-center px-6 py-12 text-center">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                Работодатель в статусе «На проверке» — появится в рассылках после подтверждения.
              </p>
              <button
                onClick={onClose}
            ...
            <form onSubmit={handleSubmit} className="space-y-5 px-6 py-5">
              ...
              >
                {status === "submitting" ? "Добавляем…" : "Добавить работодателя"}
              </button>
            </form>
          )}
        </div>
